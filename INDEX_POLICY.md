...
When a fixed technical-core category corresponds to a monorepo with multiple packages, its level-2 categories must first distinguish the repository's established top-level package domains, such as `Frontend` and `Backend`. Package-specific categories belong beneath their domain. A publication that concerns multiple packages within one domain may appear directly under that domain; it must not be attached to one package merely for placement. If the repository has no established package domains, level 2 may distinguish packages directly.
...
4. For a multi-package monorepo category, place the document under its established package domain and then its owning package when applicable before choosing purpose-based subcategories; otherwise choose the smallest useful subcategory based on the document's audience and purpose.
